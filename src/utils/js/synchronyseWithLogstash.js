...
const linux_synchronize = async(event) => {
  ...
  const isExecutedCmd = 'python '+loc.script_windows+' 8 127.0.0.1 9600'
  ...
  const isExecuted = await exec(isExecutedCmd).toString('utf-8').match(/False/g) != null ? false : true
  ...
  const windows_synchronyse = async(event) =>{
    const user = await exec("quser").toString('utf-8').match(/>([A-Z,a-z]+)/g)[0].replace(/>/g, '')

    global.configName = "logstash-config-windows.conf"

    // Find logstash
    const cmdFindLogstash = 'python '+loc.script_windows+' 2 '+'C:\\Users\\'+user
    const data = await exec(cmdFindLogstash)
    ...
    const isExecutedCmd ='python '+loc.script_windows+' 8 127.0.0.1 9600'
    event.reply('lookingForLogstash', 'Logstash found')
    const isExecuted = data.toString('utf-8').match(/False/g) != null ? false : true

    // Is logstash is runned from outside
    if (!isExecuted){
      global.logstash_process = exec('python '+loc.script_windows+' 1 '+global.logstash_dir+' '+global.configName)
      controller_logstash()
    }
  }
